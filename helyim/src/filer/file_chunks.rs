...
fn view_from_visible_intervals(
    visibles: Vec<VisibleInterval>,
    offset: i64,
    size: u64,
) -> Vec<ChunkView> {
    ...
    visibles.into_iter().for_each(|visible| {
        if visible.start <= offset && offset < visible.stop && offset < stop {
            let is_full_chunk =
                visible.is_full_chunk && visible.start == offset && visible.stop == stop;
...
fn merge_into_visibles(
    visibles: &mut Vec<VisibleInterval>,
    chunk: &FileChunk,
) -> Vec<VisibleInterval> {
