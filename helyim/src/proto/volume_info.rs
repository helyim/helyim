...
use crate::{anyhow, errors::Result, util::check_file};

pub fn maybe_load_volume_info(filename: &str) -> Result<Option<VolumeInfo>> {
...
pub fn save_volume_info(filename: &str, volume_info: VolumeInfo) -> Result<()> {
    match check_file(filename)? {
        Some((_, false, _, _)) => {
