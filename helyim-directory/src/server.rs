...
impl DirectoryServer {
    pub async fn new(
        ...
        sequencer: Sequencer,
    ) -> Result<DirectoryServer, DirectoryError> {
...
impl Helyim for DirectoryGrpcServer {
    ...
    async fn assign(
        ...
        let replica_placement = ReplicaPlacement::new(&request.replication)?;
        let ttl = Ttl::new(&request.ttl).map_err(|err| Status::internal(err.to_string()))?;
        ...
        if !self.topology.has_writable_volume(&volume_grow_option).await {
            ...
            let automatic_grow_by_type = self
                ...
                .await;
            automatic_grow_by_type.map_err(|err| Status::internal(err.to_string()))?;
        }
        ...
        let pick_for_write = self
            ...
            .await;
        let (fid, count, data_node) =
            pick_for_write.map_err(|err| Status::internal(err.to_string()))?;
