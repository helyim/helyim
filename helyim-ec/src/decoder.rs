...
pub fn write_index_file_from_ec_index(base_filename: &str) -> Result<(), io::Error> {
    ...
    iterate_ecj_file(
        base_filename,
        Some(|needle_id| -> StdResult<(), io::Error> {
...
pub fn find_data_filesize(base_filename: &str) -> Result<u64, io::Error> {
    // TODO: handle version
    ...
    iterate_ecx_file(
        base_filename,
        Some(
            |_needle_id, offset: Offset, size: Size| -> StdResult<(), io::Error> {
                if size.is_deleted() {
                    return Ok(());
                }
...
fn iterate_ecx_file<F>(base_filename: &str, mut process_needle: Option<F>) -> Result<(), io::Error>
where
    F: FnMut(NeedleId, Offset, Size) -> StdResult<(), io::Error>,
{
    let mut ecx_file = fs::OpenOptions::new()
        .read(true)
        .mode(0o644)
...
fn iterate_ecj_file<F>(base_filename: &str, mut process_needle: Option<F>) -> Result<(), io::Error>
where
    F: FnMut(NeedleId) -> StdResult<(), io::Error>,
{
...
pub fn write_data_file(base_filename: &str, mut data_filesize: i64) -> Result<(), io::Error> {
