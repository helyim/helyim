...
impl VolumeServer {
    pub async fn new(
        ...
        read_redirect: bool,
    ) -> Result<VolumeServer, VolumeError> {
    ...
    pub async fn stop(self) -> Result<(), VolumeError> {
        self.shutdown.broadcast(()).await?;
        Ok(())
    }

    pub async fn start(&mut self) -> Result<(), VolumeError> {
    ...
    async fn do_heartbeat(
        ...
        let request_stream = stream! {
            loop {
                tokio::select! {
                    _ = interval.tick() => {
                        yield store_ref.collect_heartbeat();
