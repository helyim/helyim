...
impl Store {
    ...
    pub async fn read_volume_needle(
        &self,
        vid: VolumeId,
        needle: &mut Needle,
        ...
        match self.find_volume(vid) {
            ...
            None => Err(io::Error::new(
                ErrorKind::NotFound,
                format!("Volume {vid} is not found"),
            ))?,
        }
    }

    pub async fn write_volume_needle(
        &self,
        vid: VolumeId,
        needle: &mut Needle,
    ...
    async fn find_free_location(&self) -> Option<&DiskLocation> {
    ...
    async fn do_add_volume(
        ...
        let location = self
            .find_free_location()
            .await
            .ok_or::<VolumeError>(anyhow!("no more free space left"))?;
    ...
    pub fn collect_heartbeat(&self) -> HeartbeatRequest {
    ...
    pub fn check_compact_volume(&self, vid: VolumeId) -> Result<f64, VolumeError> {
        match self.find_volume(vid) {
            Some(volume) => {
    ...
    pub fn compact_volume(&self, vid: VolumeId, _preallocate: u64) -> Result<(), VolumeError> {
        match self.find_volume(vid) {
            Some(volume) => {
    ...
    pub fn commit_compact_volume(&self, vid: VolumeId) -> Result<(), VolumeError> {
    ...
    pub fn commit_cleanup_volume(&self, vid: VolumeId) -> Result<(), io::Error> {
        match self.find_volume(vid) {
            Some(volume) => {
            ...
            None => Err(io::Error::new(
                ErrorKind::NotFound,
                format!("volume {vid} is not found during cleaning up."),
            )),
        }
    }
}
